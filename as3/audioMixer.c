...
void AudioMixer_init(void)
{
	...
	AudioMixer_setTempo(DEFAULT_TEMPO);
...
void sleepForTempo(){
	//printf("tempo is %d \n",AudioMixer_getTempo());
    long seconds = 0;
	long nanoseconds = ((60/(double)tempo/2)*1000*1000000);
...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	for(int i=0;i<MAX_SOUND_BITES;i++){
		if(i==MAX_SOUND_BITES-1){
			printf("Error: Can't found free slot for insert the sound\n");		
		}
		if(soundBites[i].pSound==NULL){
			//printf("free slot founded at #%d ",i);
...
void AudioMixer_volumeUp(){
	...
	//if(volume<=AUDIOMIXER_MAX_VOLUME-5){
		volume+=5;
...
void AudioMixer_tempoUp(){
	
	//if(tempo<=AUDIOMIXER_MAX_TEMPO-5){
		tempo+=5;
		AudioMixer_setTempo(tempo);
	//}	
	
}
void AudioMixer_tempoDown(){
	
	//if(tempo>=AUDIOMIXER_MIN_TEMPO+5){
		tempo-=5;
		AudioMixer_setTempo(tempo);
	//}
	//printf("tempo is %d \n",tempo);
...
void AudioMixer_setTempo(int newTempo){
	...
	tempo=newTempo;
	if(tempo>AUDIOMIXER_MAX_TEMPO){
		tempo=AUDIOMIXER_MAX_TEMPO;
	}
	if(tempo<AUDIOMIXER_MIN_TEMPO){
		tempo=AUDIOMIXER_MIN_TEMPO;
	}
	//printf("tempo is %d \n",tempo);
	//printf("tempo is %d \n",tempo);
}
...
static void fillPlaybackBuffer(short *playbackBuffer, int size)
{
	 ...
	 *
	 */
	//printf("--------------------------------in fill playback buffer\n");
	...
	for(int i=0;i<MAX_SOUND_BITES;i++){
		if(soundBites[i].pSound!=NULL){
			//printf("fill\n");
			//copy to local to increase performance
			int offset=soundBites[i].location;
			int samples=soundBites[i].pSound->numSamples;
			for(int j=0;j<size;j++){
				//not at the end of the file
				if(offset<samples){
					added_pcm=playbackBuffer[j]+soundBites[i].pSound->pData[offset];
					if(added_pcm>=SHRT_MAX){
						added_pcm=SHRT_MAX;
					}
					if(added_pcm<=SHRT_MIN){
						added_pcm=SHRT_MIN;
					}
					//convert back to short
					playbackBuffer[j]=(short)added_pcm;
				}
				else {
					soundBites[i].location = 0;
					soundBites[i].pSound = NULL;
